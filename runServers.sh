  ...
  case "$OPTION" in
    ...
    n)
      ...
      do
        ...
        docker run -d --name "server$instance" -e "PASSWORD=admin" -p "$port:8080" --network="sec-network" -v "$(pwd)/GeneratedKeyStores/server${instance}keystore.jks:/app/serverkeystore.jks" -v "$(pwd)/GeneratedKeyStores/serversInfo.txt:/app/serversInfo.txt" server
