  ...
  case "$OPTION" in
    ...
    n)
      ...
      do
        ...
        docker run -d --name "server$instance" -e "PASSWORD=admin" -p "$port:8080" --network="sec-network" -v "$(pwd)/GeneratedKeyStores/server${instance}keyStore.jks:/app/serverkeystore.jks" server
